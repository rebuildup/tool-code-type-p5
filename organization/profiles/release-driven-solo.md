...
tool-code-type-p5 の current release-driven development で使用する Operating Model を定義します。
...
- Existing AGENTS/ADR hidden sibling dependency and client-only p5 semantics remain project-specific authority.
- standalone checkout cannot prove full typecheck/bundle integration while the sibling UI dependency is unresolved.
- Bun is used for project-local dependency/Skill operations where existing project docs permit it; host validation remains a separate evidence stage.
